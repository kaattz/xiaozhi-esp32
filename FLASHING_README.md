...
本文档记录在 Windows PowerShell 下，把 `xiaozhi-esp32` 烧录到 ESP32-S3 开发板的流程。默认以太极小派为例，ESP32-S3-BOX-3B 和 Home Assistant Voice PE 只需要按本文的差异配置修改板型和硬件选项。
...
如果你在当前 Voice PE 适配仓库里操作，目录是：

```powershell
cd C:\Code\xiaozhi-voice-pe
```
...
Home Assistant Voice PE 当前这块板之前识别为 `COM8`。如果设备管理器里显示不同串口，后续命令把 `COM8` 换成实际串口。
...
```text
Xiaozhi Assistant -> Board Type -> Home Assistant Voice Preview Edition
```
...
| Default Language | Chinese |
| Wake Word Implementation Type | Wakenet model with AFE |
| WakeNet model | `你好小智` / `CONFIG_SR_WN_WN9_NIHAOXIAOZHI_TTS` |
| Send Wake Word Data | 开启 |
| Xiaozhi Assistant -> Enable Device-Side AEC | 开启 |

也可以不用手动进 `menuconfig`，直接用板型参数编译：

```powershell
idf.py -DBOARD_NAME=home-assistant-voice-pe -DBOARD_TYPE=home-assistant-voice-pe build
```
...
Voice PE 也是 16MB Flash，可以用同样的完整 `esptool` 命令。前提是你已经在 Voice PE 配置下完成过 `build`。
...
| Voice PE 唤醒没反应 | 先确认 mute 开关没有打开，再看日志有没有 `Wake word detected: 你好小智` |
| Voice PE 想重新配网 | 长按中间键，连接 `Xiaozhi-XXXX` 后打开 `http://192.168.4.1` |
