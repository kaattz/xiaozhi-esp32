# Wake Arbitration 方案对比：wake_group + 音量 与音频指纹
...
把容易互相串音的设备放进同一个 `wake_group`。同一组内多个设备被唤醒时，gateway 等一个很短的窗口，收集这些唤醒请求，然后选唤醒音量最大的设备。
...
| 同组 500ms 内只有一个设备唤醒 | 允许该设备 |
| 同组 500ms 内多个设备唤醒 | 选 `wake_volume` 最大的 |
...
| 项目 | 说明 |
|---|---|
| 实现简单 | ESP32 只需要多上报一个 `wake_volume` |
| 速度稳定 | 不需要上传或分析唤醒音频 |
...
| 指纹提取失败 | 不继续猜，按 `wake_group + 音量` 规则处理 |
...
| 维度 | wake_group + 音量 | gateway 音频指纹 |
|---|---|---|
| 推荐阶段 | 第一版 | 第二版增强 |
| 实现难度 | 低 | 中 |
| 新增延迟 | 约 300-550ms | 约 500-900ms |
| 是否传音频到 gateway | 否 | 是 |
...
第一阶段先做 `wake_group + 音量`：

```text
多设备组：等 500ms，选 wake_volume 最大
单设备组：直接允许
不同组：互不阻塞
```
...
先不要直接上音频指纹。第一版优先实现 `wake_group + wake_volume + priority`，因为它速度快、实现少、失败面小，已经能解决大部分“开门串音误唤醒”问题。
